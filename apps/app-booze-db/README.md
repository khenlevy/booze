...
2. **Access MongoDB**:
   ...
   ssh -L 27017:localhost:27017 buydy-droplet
...
3. **Check MongoDB logs**:
   ```bash
   ssh buydy-droplet "docker logs mongo"
   ```
...
1. **Check backup directory**:
   ...
   ```bash
   ssh buydy-droplet "ls -la /var/backups/mongo/"
   ```

2. **Manual backup**:
   ```bash
   ssh buydy-droplet "cd /opt/app-db && ./scripts/backup_mongo.sh"
   ```
