  ...
  try {
    ...
    try {
      ...
      await executeRemoteCommand(
        sshConn,
        `docker run --rm -v /tmp:/workspace -v /opt/app-booze-db/docker/.env.production:/workspace/.env.production --network container:mongo -e NODE_ENV=production -e DOCKER_RUNNING=true -e MONGO_HOST=localhost node:18-alpine sh -c "cd /workspace && npm install -g dotenv-cli && dotenv -e .env.production node create-optimal-indexes.js"`,
      );
