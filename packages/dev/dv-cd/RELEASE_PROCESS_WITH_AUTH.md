...
ssh buydy-droplet "docker exec mongo mongosh --authenticationDatabase admin -u admin -p 'j3Sh01ZaSDFj3i854YF5sa' --eval 'db.adminCommand(\"ping\")'"

# Check firewall rules
ssh buydy-droplet "ufw status | grep 27017"
...
1. **Reset MongoDB with fresh authentication:**
   ```bash
   ssh buydy-droplet "docker stop mongo && docker rm mongo && docker volume rm mongo_data"
   yarn release:db
   ```
...
2. **Check if MongoDB is running with authentication:**
   ```bash
   ssh buydy-droplet "docker ps | grep mongo"
   ssh buydy-droplet "docker logs mongo --tail 5"
   ```
...
1. **Stop all services:**
   ```bash
   ssh buydy-droplet "docker stop \$(docker ps -q)"
   ```

2. **Reset MongoDB completely:**
   ```bash
   ssh buydy-droplet "docker rm \$(docker ps -aq) && docker volume rm mongo_data"
   ```
