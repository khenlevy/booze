...
export async function deployApp(conn, basePath, releasePath, imageName) {
  ...
  const portBinding =
    imageName === "app-booze-scanner"
      ? "127.0.0.1:4001:4001" // Scanner: localhost only
      : imageName === "app-booze-api"
  ...
  const memoryLimit = imageName === "app-booze-scanner" ? "1100m" : "512m";
  const memorySwap = imageName === "app-booze-scanner" ? "2200m" : "1g";
