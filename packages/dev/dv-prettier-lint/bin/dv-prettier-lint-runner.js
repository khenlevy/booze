...
function detectContext(targetDir) {
  ...
  // Mobile/Expo apps: use package-level config (eslint.config.js, .prettierrc)
  if (deps.expo || name.includes("mobile")) return "mobile";
...
try {
  ...
  // Mobile: use package-level config (expo lint + local prettier)
  if (context === "mobile") {
    console.log(`\n🔍 Running ESLint (expo lint) ...`);
    try {
      execSync("npx expo lint", { stdio: "inherit", cwd });
    ...
    // Use package-level .prettierrc if present, else fallback to client config
    const localPrettierCjs = path.join(cwd, ".prettierrc.cjs");
    const localPrettierJson = path.join(cwd, ".prettierrc.json");
    const fallbackPrettier = path.join(
      __dirname,
      "../config/client/.prettierrc.cjs"
    );
    const prettierConfigPath = fs.existsSync(localPrettierCjs)
      ? localPrettierCjs
      : fs.existsSync(localPrettierJson)
        ? localPrettierJson
        : fallbackPrettier;

    console.log(`\n🎨 Running Prettier ...`);
    try {
      const prettierCmd = `npx prettier --config "${prettierConfigPath}" --write "**/*.{js,jsx,json,css,md}"`;
      execSync(prettierCmd, { stdio: "inherit", cwd });
