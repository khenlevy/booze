...
/**
 * Installs Docker on the remote server using the installation script
 ...
 */
export async function installDocker(conn, options = {}) {
  ...
  try {
    ...
    await conn.execCommand(`chmod +x ${tempScriptPath}`);
    ...
    try {
      await executeRemoteCommand(conn, `rm -f ${tempScriptPath}`, {
        throwOnError: false,
        silent: true,
      });
    } catch (cleanupError) {
